...
export default function PaymentModal({ 
  isOpen, onClose, onPay, amount, loading, 
  profileType = 'Etudiant', totalFormations = 1 
}: PaymentModalProps) {
  ...
  const handlePay = () => {
    if (selectedMethod) onPay(selectedMethod);
  };
  ...
  const getDiscountLabel = () => {
    if (profileType === 'Etudiant') return 'Prix bourse';
    if (profileType === 'Stagiaire') return 'Tarif préférentiel';
    return `Réduction ${getProDiscount()}%`;
  };

  const paymentMethods = [
    { id: 'wave' as const, name: 'Wave', icon: Smartphone, bgColor: 'bg-blue-500/10', borderColor: 'border-blue-500/20', textColor: 'text-blue-400', description: 'Paiement mobile instantané' },
    { id: 'paypal' as const, name: 'PayPal', icon: CreditCard, bgColor: 'bg-indigo-500/10', borderColor: 'border-indigo-500/20', textColor: 'text-indigo-400', description: 'Paiement sécurisé en ligne' },
    { id: 'bank' as const, name: 'Virement Bancaire', icon: Building2, bgColor: 'bg-purple-500/10', borderColor: 'border-purple-500/20', textColor: 'text-purple-400', description: 'Virement classique 24-48h' },
  ];
    ...
    >
      ...
      >
        ...
        <div className="relative p-6 border-b border-[#1e293b]">
          <button onClick={handleClose} className="absolute right-4 top-4 p-2 hover:bg-[#1e293b] rounded-xl transition-colors group">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Paiement sécurisé • 100% crypté</p>
            </div>
          </div>
        </div>
            ...
            {step === 'select' ? (
              <motion.div key="select" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                ...
                <div className="bg-[#020617] border border-[#1e293b] rounded-xl p-4 text-center">
                  ...
                  <p className="text-xs text-slate-500 mt-1">{formatEUR(amount)}</p>
                  
                  <div className="flex items-center justify-center gap-1 mt-2">
                    {profileType === 'Etudiant' && (
                      <>
                        <Sparkles className="w-3 h-3 text-blue-400" />
                        <p className="text-[10px] text-blue-400 font-medium">Tarif bourse étudiant</p>
                      </>
                    )}
                    {profileType === 'Stagiaire' && (
                      <>
                        <Sparkles className="w-3 h-3 text-green-400" />
                        <p className="text-[10px] text-green-400 font-medium">Tarif préférentiel</p>
                      </>
                    )}
                    {profileType !== 'Etudiant' && profileType !== 'Stagiaire' && (
                      <>
                        ...
                        <p className="text-[10px] text-amber-400 font-medium">
                          {getDiscountLabel()} • {totalFormations} formation{totalFormations > 1 ? 's' : ''}
                        </p>
                      </>
                    )}
                  </div>
                  ...
                  {paymentMethods.map((method) => (
                    ...
                    >
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                        selectedMethod === method.id ? 'border-green-400 bg-green-400' : 'border-slate-600'
                      }`}>
                        {selectedMethod === method.id && (
                ...
                <motion.button
                  ...
                  onClick={handleConfirm} disabled={!selectedMethod}
                ...
                <div className="flex items-center justify-center gap-2 text-[10px] text-slate-500">
                  ...
                  <Shield className="w-3 h-3" /> Protection acheteur
                </div>
              </motion.div>
            ) : (
                ...
                <div className="bg-[#020617] border border-[#1e293b] rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-[#1e293b]">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-xs text-slate-500">{formatEUR(amount)}</span>
                    </div>
                  </div>
                  {profileType !== 'Etudiant' && profileType !== 'Stagiaire' && (
                    <div className="flex justify-between items-center pt-2 border-t border-[#1e293b]">
                      <span className="text-xs text-green-400 flex items-center gap-1">
                        <TrendingUp className="w-3 h-3" /> {getDiscountLabel()}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    ...
                    {loading ? (
                      ...
                      <><Lock className="w-4 h-4" /> Payer {amount.toLocaleString()} FCFA</>
                    )}
                  </motion.button>
                </div>
              </motion.div>
        ...
        <div className="px-6 py-3 border-t border-[#1e293b] bg-[#020617]/50">
          <div className="flex items-center justify-center gap-4">
            <div className="flex items-center gap-1.5"><Shield className="w-3 h-3 text-green-400" /><span className="text-[10px] text-slate-500">SSL</span></div>
            <div className="w-px h-3 bg-[#1e293b]" />
            <div className="flex items-center gap-1.5"><Lock className="w-3 h-3 text-green-400" /><span className="text-[10px] text-slate-500">Crypté</span></div>
            <div className="w-px h-3 bg-[#1e293b]" />
            <div className="flex items-center gap-1.5"><Check className="w-3 h-3 text-green-400" /><span className="text-[10px] text-slate-500">Instantané</span></div>
          </div>
        </div>
      </motion.div>
    </motion.div>
