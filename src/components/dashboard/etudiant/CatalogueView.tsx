          ...
          {filteredCertificates.map((cert, index) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -4 }}
                className={`group relative bg-[#0f172a] border rounded-2xl p-5 transition-all duration-300 ${
                  isPaid 
              ...
              >
                ...
                <div className="absolute top-3 right-3">
                  {isPaid && (
                    <motion.span
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="inline-flex items-center gap-1 px-2 py-1 bg-green-500/10 border border-green-500/20 rounded-full"
                    >
                      <CheckCircle2 className="w-3 h-3 text-green-400" />
                      <span className="text-[10px] font-medium text-green-400">Accès obtenu</span>
                    </motion.span>
                  )}
                  {isPending && (
                    <motion.span
                      initial={{ scale: 0 }}
                      animate={{ scale: 1 }}
                      className="inline-flex items-center gap-1 px-2 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full"
                    >
                      <AlertCircle className="w-3 h-3 text-amber-400" />
                      <span className="text-[10px] font-medium text-amber-400">En attente</span>
                    </motion.span>
                  )}
                </div>
                ...
                <div className={`p-2.5 rounded-xl w-fit mb-4 ${
                  isPaid ? 'bg-green-500/10' : isPending ? 'bg-amber-500/10' : 'bg-blue-500/10'
                }`}>
                  <GraduationCap className={`w-5 h-5 ${
                    isPaid ? 'text-green-400' : isPending ? 'text-amber-400' : 'text-blue-400'
                  }`} />
                </div>
                ...
                <p className="text-xs text-slate-400 mb-4 line-clamp-2">
                  {cert.description || 'Formation complète avec certification à la clé.'}
                </p>
